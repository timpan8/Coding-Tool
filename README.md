...
Ett projekt kan ha flera filer. Versioner har etiketter (en tom etikett blir raddiffen, `+12 −3`), går att jämföra, förhandsgranska skrivskyddat och radera. I editorn läses en platshållare som ett piller med AI-värdet som badge; badgen öppnar ett kort som visar det riktiga värdet på begäran i en minut, redigerar bindingen eller skriver tillbaka värdet på just den platsen. Bindings kan byta namn i alla mallar på en gång, raderas med värdet återskrivet i filen, ha ett värde per profil, och nås samlat på `#/bindings` — vilket är enda stället en global binding går att komma åt. Kod som kommer tillbaka från en AI kan klistras in och matchas mot dina värden igen, i tre nivåer där den tredje alltid är ett förslag.

Före en AI-kopia letas kända värden inte bara i klartext: base64 (även UTF-16 och inuti längre körningar), URL-, JSON- och HTML-escaping, backtick och regex-escaping räknas som samma värde. Värden en binding har haft tidigare bevakas för alltid, och en binding vars riktiga värde dyker upp i kod som klistras in från en AI märks som exponerad tills du byter det. Kopiera RIKTIGT märker texten med en rad som säger att den bär riktiga värden. Sökvägar kan skrivas mot en rot (`{{ROOT}}\AdSync`) och följer med när du byter arbetsmapp.
...
**IndexedDB lagrar privata värden i klartext som standard.** Kryptering går att slå på under Inställningar: ett lösenord (PBKDF2-SHA256, 600 000 varv) packar in en slumpad datanyckel, en återställningsnyckel visas en gång, och varje post krypteras med AES-256-GCM. Id:n och tidsstämplar ligger kvar i klartext, eftersom databasen måste kunna hitta en rad utan nyckel. Låst valv visar en låsskärm och ingenting annat; auto-lås efter inaktivitet är valbart. Krypteringen skyddar valvet när webbläsaren är stängd — inte mot ett tillägg som läser sidan medan du arbetar, och inte mot vad du själv kopierar. Tappar du både lösenordet och återställningsnyckeln finns ingen väg tillbaka. Rensad webbläsardata, annan profil eller annat origin kan göra valvet oåtkomligt — exportera en backup regelbundet. Exportfilen från ett krypterat valv krypteras med samma lösenord och återställningsnyckel; från ett okrypterat valv är den klartext, och den privata varianten utelämnar projektkoden men inte värdena.
